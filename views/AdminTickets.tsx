                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar px-6 py-6 space-y-6 bg-slate-50/30">
                   <div className="max-w-3xl mx-auto space-y-6">
                      ...
                      <div 
                        ...
                        className="bg-white rounded-2xl shadow-sm border border-slate-200 cursor-pointer overflow-hidden transition-all duration-300"
                      >
                         <div className="p-4 flex items-center justify-between gap-6">
                            <div className="flex-grow min-w-0">
                               <div className="flex items-center gap-1.5 text-heritage-red mb-1">
                                  <Sparkles size={10} />
                                  <span className="text-[8px] font-black uppercase tracking-[0.2em]">AI Intelligence (Click to Expand)</span>
                               </div>
                            ...
                            <div className="flex items-center gap-4 shrink-0 border-l border-slate-100 pl-4">
                               ...
                               <ChevronDown size={14} className={`text-slate-400 transition-transform ${isAiExpanded ? 'rotate-180' : ''}`} />
                            </div>
                         </div>
                         
                         {isAiExpanded && (
                           <div className="px-4 pb-6 pt-2 border-t border-slate-100 bg-slate-50/50 space-y-6 animate-in slide-in-from-top-2">
                              <div className="grid grid-cols-2 gap-8">
                                 <div>
                                    <h4 className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3">Risk Assessment</h4>
                                    <p className="text-[11px] font-bold text-heritage-maroon leading-relaxed">
                                       {selectedTicket.analysis?.intelligenceBriefing?.riskAssessment || "No risks detected."}
                                    </p>
                                 </div>
                                 <div>
                                    <h4 className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3">Logistics Audit</h4>
                              ...
                              <div>
                                 <h4 className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3">Investigation Strategy</h4>
                                 <ul className="space-y-2">
                                    {selectedTicket.analysis?.intelligenceBriefing?.investigationStrategy?.map((step, i) => (
                                      <li key={i} className="flex gap-2 items-center text-[10px] font-bold text-heritage-ink">
                                         <div className="w-1 h-1 bg-heritage-red rounded-full" />
                                         {step}
                                      </li>
                                    ))}
                                 </ul>
